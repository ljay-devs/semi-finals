...
app.get("/GET/products", (req, res) =>{
    //create a query
    connection.query("SELECT * FROM bdshop",(err, rows, fields)=>{
        //checking errors
        if(err) throw err;
        //response
        //key value pair
...
app.post("/POST/products", (req, res)=>{
   const {itemname, unitprice, quantity, supplier} = req.body;
    connection.query(`INSERT INTO dbshop (itemName, unitPrice, quantity, supplier) VALUES ('${itemname}','${unitprice}', '${quantity}', '${supplier}')`, (err, rows, fields) =>{
        if(err) throw err;
